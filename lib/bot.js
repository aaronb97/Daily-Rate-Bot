...
const CARD_RANGE = 'Cards!A1:Z1000';
...
class Bot {
    ...
    static checkMessage(message) {
        ...
        if (arr.length >= 2 && arr.length <= 5 && messageText.length < 30 && arr[0] == "/rate") {
            ...
            if (arr.length >= 3 && (arr[1] == "light" || arr[1] == "decent" || arr[1] == "solid" || arr[1] == "strong") && parseInt(arr[2]) != NaN)
            {
                var nameSplit = message.name.split(" ");
                if (nameSplit.length == 1)
                    Bot.sendMessage("Nice one, " + message.name + "thony " + message.name + "tano ");
                if (nameSplit.length >= 2)
                    Bot.sendMessage("Nice one, " + nameSplit[0] + "thony " + nameSplit[1] + "tano ");
                arr[1] = arr[2];
            }
            ...
            googleAuth.authorize()
                .then((auth) => {
                    ...
                    sheets.spreadsheets.values.get({
                        ...
                        var valid = true;
                        user2name = user2name.replace(/\s/g, '');;
                        var rows = response.values;
                        var x;
                        for (x = 0; rows[0][x] != message.user_id && rows[0][x] != undefined; x++);
                        var y;
                ...
                .then((auth) => {
                    ...
                    sheets.spreadsheets.values.get({
                        ...
                        for (y = 0; rows[y][TRADE_INDEX] != undefined && rows[y][TRADE_INDEX] != ""; y++)
                        {
                            ...
                            {
                                returnmessage += count + ": " + rows[y][TRADE_INDEX] + "\n";
                                count++;
                            }
                        }
                ...
                .then((auth) => {
                    ...
                    sheets.spreadsheets.values.get({
                        ...
                        for (z = 0; z < cards1.length && valid == true; z++)
                        {
                            ...
                            {
                                valid = false;
                                console.log("Do not have " + cardamount + " " + rows[q][0]);
                            }
                            card1indexes.push(q);
                        ...
                        for (z = 0; z < cards2.length && valid == true; z++)
                        {
                            ...
                            {
                                valid = false;
                                Bot.sendMessage("Do not have " + cardamount + " " + rows[q][0]);
                            }
                            card2indexes.push(q);
                        ...
                        if (valid == true)
                        {
                            ...
                            Bot.sendMessage("Completed trade");
                        }
            ...
            googleAuth.authorize()
                .then((auth) => {
                    ...
                    sheets.spreadsheets.values.get({
                        ...
                        var cardPicks = 1;
                        if (arr.length > 1 && arr[1] != "rare" && parseInt(arr[1]) != NaN)
                        ...
                        else
                        {
                            ...
                            for (z = 0; z < cardPicks; z++)
                            {
                                ...
                                r = Math.pow(r / parseInt(rows[1][3]), 1 / Math.pow(rarity, 1.5)) * parseInt(rows[1][3]);
                    ...
                    sheets.spreadsheets.values.get({
                        ...
                        if (arr[0] == "/cards")
                        {
                            ...
                            cardarray.sort(function(a, b){
                                if (a.includes("foil") && !b.includes("foil"))
                                    return -1;
                                else if (!a.includes("foil") && b.includes("foil"))
                                    return 1; 
                                else
        ...
        googleAuth.authorize()
            .then((auth) => {
                ...
                sheets.spreadsheets.values.get({
                    ...
                    if (parseInt(rows[4][x]) + 3 < y && arr[0] == "/rate" && (rows[y - 1][x] == undefined || rows[y - 1][x] == "" || rows[y - 2][x] == undefined || rows[y - 2][x] == "" || rows[y - 3][x] == undefined || rows[y - 3][x] == ""))
                        extra += "\nYou have some missing rates in the past few days, do /missing and /ratedate # mm/dd/yy";
        ...
        googleAuth.authorize()
            .then((auth) => {
                ...
                sheets.spreadsheets.values.get({
                    ...
                    if (err) {
                        console.log('The API returned an error: ' + err);
                        Bot.sendMessage("Error with ratealert " + err.code);
                        return;
                    }
                    var rows = response.values;
                    var x;
