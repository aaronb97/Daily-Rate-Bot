...
class Bot {
    ...
    static checkMessage(message) {
        ...
        if (arr.length >= 2 && messageText && arr[0] == "/rate") {
            var rate = parseInt(arr[1]);
            if (isNaN(rate) || rate != parseFloat(arr[1]) || rate < 0 || rate > 10 || arr[1].length > 2)
                return "You are not allowed to rate " + arr[1];
            else
            {
                Bot.rateEnter(message.user_id, name, rate, date, daterate);
        ...
        {
            console.log("Arg 1: " + arr[0] + " Arg 2: " + arr[1] + " Arg 3: " + arr[2]);
            var rate = parseInt(arr[1]);
            if (isNaN(rate) || rate != parseFloat(arr[1]) || rate < 0 || rate > 10 || arr[1].length > 2)
                return "You are not allowed to rate " + arr[1];
            ...
            Bot.rateEnter(message.user_id, name, rate, date2, daterate);
            console.log("Logged rate of " + arr[1] + " for " +  name + " on " + daterate)
        }
        else if (arr.length >= 2 && messageText && missing.test(messageText) && arr[1] == "all")
        {
                    ...
                    sheets.spreadsheets.values.get({
                        ...
                        for (w = y; w > 2; w--)
                        {
                            ...
                            for (z = 1; z < x; z++)
                            {
                                ...
                                if ((rows[w][z] == undefined || rows[w][z] == "") && (rows[0][z] != 17700160 || w > 32)  )
                                {
        ...
        else if (arr.length == 1 && messageText && missing.test(messageText))
        {
            require('dotenv').config();
            const google = require('googleapis');
            const sheetsApi = google.sheets('v4');
            ...
            googleAuth.authorize()
                .then((auth) => {
                    ...
                    sheets.spreadsheets.values.get({
                        ...
                        for (z = 2; z <= y; z++)
                        {
                            if ((rows[z][x] == undefined || rows[z][x] == "" ) && (rows[0][x] != 17700160 || z > 32))
                            {
                        ...
                        if (rows[0][x] == "" || rows[0][x] == undefined)
                            ...
                            Bot.sendMessage("You are up to date");
        ...
        {
            return "/rate # \n\n /ratedate # mm/dd/yyyy\n   (leading zeros not required) \n\n /link\n(Links the spreadsheet)\n\n/missing\n(Lists missing dates for you)\n\n/missing all\n(Lists missing dates for everyone)\n\n/speak";
        }
            ...
            googleAuth.authorize()
                .then((auth) => {
                    ...
                    sheets.spreadsheets.values.get({
                        ...
                        {
                            numbers[Math.floor(Math.random() * 4)] = Math.floor(Math.random() * 60 + 1);    
                        }
                        numbers.sort(function(a, b){return a-b});
                        var powerball = Math.floor(Math.random() * 24) + 1;
            ...
            googleAuth.authorize()
                .then((auth) => {
                    ...
                    sheets.spreadsheets.values.get({
                        ...
                        if (rows[0][x] != undefined && rows[0][x] != "" && (rows[y][x] == undefined || rows[y][x] == "") && d.getHours() > 7)
                        {
...
static rateEnter(id, name, rate, d, daterate)
{
        ...
        .then((auth) => {
            ...
            sheets.spreadsheets.values.get({
                ...
                if (rows[y][x] == undefined || rows[y][x] == "")
                    Bot.sendMessage("Logged rate of " + rate + " for " +  name + " for " + daterate); 
                else if (rows[y][x] == rate)
                    Bot.sendMessage("You have already logged this rate");  
                else
