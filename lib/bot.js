...
class Bot {
    ...
    static checkMessage(message) {
        ...
        if (splitMessage.length >= 2 && splitMessage.length <= 5 && messageText.length < 30 && (splitMessage[0] == "/rate" || splitMessage[0] == "/r")) {
            ...
            {
                let dateString =  moment().startOf('day').add(-1, 'days').tz("America/New_York").format("M/D/YY");
        ...
        else if (splitMessage[0] == "/ratedate" || splitMessage[0] == "/daterate" || splitMessage[0] == "/rd")
        {
            if (splitMessage.length < 3)
            ...
            if (dateTest != undefined && dateTest.length >= 2)
            {
            ...
            //add the year to the date if there is only one forward slash
            var datesplit = splitMessage[2].split("/");
            if (datesplit.length == 2)
            {
                datesplit.push(moment().add(-1, 'days').tz("America/New_York").format('YY'));
                splitMessage[2] = datesplit[0] + "/" + datesplit[1] + "/" + datesplit[2];
            }
            else if (datesplit.length == 3 && parseInt(datesplit[2]) > 2000)
            {
                datesplit[2] = parseInt(datesplit[2]) - 2000;
                splitMessage[2] = datesplit[0] + "/" + datesplit[1] + "/" + datesplit[2];
            }

            let parsedDateObject = moment(splitMessage[2], ["MM/DD/YY", "M/DD/YY", "M/D/YY", "MM/D/YY"], true);
            ...
            let currentDateObject = moment().tz("America/New_York");

            if (parsedDateObject.format('M/D/YY') == currentDateObject.format('M/D/YY'))
            {
                return "You may not rate today until the day is through";
            }
            ...
            googleAuth.authorize()
                .then((auth) => {
                    ...
                    sheets.spreadsheets.values.get({
                        ...
                        let dateString =  moment().add(-1, 'days').tz("America/New_York").format("M/D/YY");
        ...
        else if (splitMessage[0] == "/tdd" || splitMessage[0] == "/tdt")
        {
            googleAuth.authorize()
                .then((auth) => {
                    ...
                    sheets.spreadsheets.values.get({
                        ...
                        {
                            let date =  moment().add(-1, 'days').tz("America/New_York");
                            let threshold = random(date.day() + 1);
                            let randomNum = Math.random();
                            
                            if (randomNum >= threshold)
                            {
                                if (splitMessage[0] == "/tdd")
                                {
                                ...
                                {
                                    Bot.sendMessage("You have tripled your points from " + points + " to " + points * 3 + "! *applause*");
                                    points *= 3;
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    Bot.sendMessage("So sorry, you lost everything and then some. You went from " + points + " down to -" + points +". Pick again");
                                    points = -points;
                                }
                            }
                            rows[2][x] = points;
        ...
        googleAuth.authorize()
            .then((auth) => {
                ...
                sheets.spreadsheets.values.get({
                    ...
                    var rows = response.values;
                    var rateCol;
                    for (rateCol = 0; rows[0][rateCol] != undefined && rows[0][rateCol] != id; rateCol++);
                    if (rows[5][rateCol] == "yes")
                    {
                        Bot.sendMessage(rows[1][rateCol] + ", your account is suspended, please consult the rate chart administrator for further details");
                        return;
                    }

                    var extra = ""; 
                    rows[0][rateCol] = id;
                    if (isEmpty(rows[1][rateCol]))
                    {
                        rows[1][rateCol] = name;
                        rows[2][rateCol] = "nag";
                        rows[3][rateCol] = "rem";
                    ...
                    if (rows[4][rateCol] == undefined || rows[4][rateCol] == "")
                        rows[4][rateCol] = rateRow;
                    ...
                    var q;
                    var sameCount = 1;
                    for (q = rateRow - 1; parseInt(rows[q][rateCol]) == rate; q--)
                        sameCount++;

                    if (parseInt(rows[4][rateCol]) + 3 < rateRow && splitMessage[0] == "/rate")
                    {
                        var ratemessage = false;
                        for (var g = 1; g <= 3; g++) 
                        {
                            if (rows[rateRow - g][rateCol] == undefined || rows[rateRow - g][rateCol] == "")
                            {
                                if (!ratemessage)
                                {
                        ...
                        var missingcount = 0;
                        for (var g = parseInt(rows[4][rateCol]); g < rateRow - 3; g++) 
                        {
                            if (rows[g][rateCol] == "" || rows[g][rateCol] == undefined)
                            {
                                missingcount++; 
                            }
                        }
                    ...
                    //Rating all of the year check
                    if (dateString.substring(0, 5) == "12/31" && isEmpty(rows[rateRow][rateCol]))
                    {
                        var yearRated = true;
                        for (var i = rateRow - 1; i > rateRow - 365; i--)
                        {
                            if (isEmpty(rows[i][rateCol]) || rows[i][rateCol] == "abstain")
                            {
                                yearRated = false; 
                            }
                        }
                        ...
                        {
                            points += 99;
                            extra += "\nGood job on rating every day of year. Enjoy an extra 99 points";
                        }
                    }

                    if (rows[rateRow][rateCol] == undefined || rows[rateRow][rateCol] == "")
                    {
                        if (isRateCommand(splitMessage[0]))
                        {
                            ...
                            if (dateString.substring(0, 4) == "3/31")
                            {
                                extra += "\nAdded " + Math.floor(Math.random() * 110 - 10) + " points"
                            }
                            else
                            {
                                extra += "\nAdded " + points + " points";
                    ...
                    else if (rows[rateRow][rateCol] == rate)
                        Bot.sendMessage(name + ", you have already logged " + rate + " for " + dateString);  
                    else
                        Bot.sendMessage("Changed rate from " + rows[rateRow][rateCol] + " to " + rate + " for " + name + " for " + dateString);

                    rows[rateRow][rateCol] = rate;
    ...
    googleAuth.authorize()
        .then((auth) => {
            ...
            sheets.spreadsheets.values.get({
                ...
                var rows = response.values;
                var rateCol;
                for (rateCol = 0; rows[0][rateCol] != id && rows[0][rateCol] != undefined; rateCol++);
                rows[2][rateCol] = points + parseInt(rows[2][rateCol]);;
                sheets.spreadsheets.values.update({
                    auth: auth,
                    spreadsheetId: SPREADSHEET_ID,
                    range: CARD_RANGE,
    ...
    googleAuth.authorize()
        .then((auth) => {
            ...
            sheets.spreadsheets.values.get({
                ...
                var ids = [];
                for (rateCol = 0; rows[0][rateCol] != undefined; rateCol++)
                {
                    if (rows[3][rateCol] != "norem" && isEmpty(rows[rateRow][rateCol]))
                    {
                        names += rows[1][rateCol] + ", ";
                        ids.push(parseInt(rows[0][rateCol]));
                    }
                    if (rows[3][rateCol] != "norem")
                    {
                        var z;
                        for (z = rateRow - 1; z >= parseInt(rows[4][rateCol]); z--)
                        {
                            if (isEmpty(rows[z][rateCol]))
                            {
                                names2 += rows[1][rateCol] + ", ";
                                ids.push(parseInt(rows[0][rateCol]));
                                z = parseInt(rows[4][rateCol]);
                            }
                        }
                    }
                }
                ...
                var message = "Bing bong it's your daily " + hours + ":" + minutes + "PM EST rate reminder\n";
