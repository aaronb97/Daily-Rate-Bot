            ...
            googleAuth.authorize()
                .then((auth) => {
                    ...
                    sheets.spreadsheets.values.get({
                        ...
                        if (valid == true)
                        {
                            Bot.sendMessage("Trade pending, waiting on " + user2name + " approval"); 
                            var x;
                ...
                .then((auth) => {
                    ...
                    sheets.spreadsheets.values.get({
                        ...
                        else
                        {
                            ...
                            {
                                Bot.sendMessage("So sorry, you lost everything. You went from " + points + " down to 0. Pick again" + r);
                                points = 0;
                            }
                            rows[2][x] = points;
                            sheets.spreadsheets.values.update({
                    ...
                    sheets.spreadsheets.values.get({
                        ...
                        else
                        {
                            ...
                            for (z = 0; z < cardPicks; z++)
                            {
                                ...
                                r = Math.pow(r / parseInt(rows[1][3]), 1 / Math.pow(rarity, 2)) * parseInt(rows[1][3]);
...
'use strict';
            ...
            .then(json => { Bot.sendMessage("Sam has most recently listened to " + json.recenttracks.track[0].name + " by " + json.recenttracks.track[0].artist["#text"]); })
        ...
        googleAuth.authorize()
            .then((auth) => {
                ...
                sheets.spreadsheets.values.get({
                    ...
                    {
                        if (arr[0] == "/rate" && (parseInt(rows[1][0]) + 1) % 1000  == 0)
                        {
